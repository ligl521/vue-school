  ...
  <div>
    <div class="schoolList" v-if="pcSee">
      ...
      <div class="schoolSimple">
        <div class="schoolId clearfix">
          <p>
            <span>NO.{{ schoolDetail.id }}</span>
          </p>
        </div>
        <div class="schoolLogo">
          ...
          <div class="schoolRight">
            <ul>
              <li>
                类型：<span>{{ schoolDetail.schoolProperties }}</span>
              </li>
              <li>
                建校时间：<span>{{ schoolDetail.foundingTime | isZero }}</span>
              </li>
            </ul>
            <ul>
              <li style="font-size:24px;">
                {{ schoolDetail.schoolName
                }}<span class="circle">{{ schoolDetail.operationState }}</span>
              </li>
              <li>{{ schoolDetail.schoolEnglishName | isZero }}</li>
              <li>
                认证：<span
                  v-for="everyApprove of approveSplit"
                  class="approve"
                  :key="everyApprove"
                  >{{ everyApprove | isZero }}</span
                >
              </li>
            </ul>
          </div>
        ...
        <div class="schoolTranslate">
          ...
          <div class="schoolTranslateBox">
            ...
            <div class="basic">
              ...
              <el-row style="margin-top: 30px;">
                ...
                <el-col :span="12">
                  <div class="grid-content bg-purple">
                    <p>
                      课程：<span
                        v-for="everyCourse of courseSplit"
                        :key="everyCourse"
                        >{{ everyCourse }}</span
                      >
                    </p>
              ...
              <el-row style="margin-top:15px;">
                <el-col :span="12">
                  <div class="grid-content bg-purple">
                    <p>
                      电话：<span>{{ schoolDetail.telephone | isZero }}</span>
                    </p>
                  </div>
                </el-col>
                <el-col :span="12">
            ...
            <div class="intoduceJeshao" ref="obj">
              <p
                :style="'height:' + activityBannerH + 'px'"
                :class="shortContent ? 'show' : 'hide'"
                class="shortContent"
              >
                {{ schoolDetail.schoolDesc }}
              </p>
              <p :class="longContent ? 'show' : 'hide'" class="longContent">
                {{ schoolDetail.schoolDesc }}
              </p>
              <span @click="lookMore" :class="more ? 'show' : 'hide'"
                >【更多】</span
              >
            </div>
          </div>
          <div class="hardware" :class="Ishardware ? 'show' : 'hide'">
            ...
            <el-row>
              <el-col :span="8"
                >占地面积(亩)：{{ schoolDetail.coveredArea | isZero }}</el-col
              >
              <el-col :span="8"
                >建筑面积(㎡)：{{ schoolDetail.builtArea | isZero }}</el-col
              >
              <el-col :span="8"
                >总容量：{{ schoolDetail.studentCapacity | isZero }}</el-col
              >
            </el-row>
            <div class="line"></div>
          ...
          <div class="teachers" :class="teacher ? 'show' : 'hide'">
            ...
            <div class="teacherBox">
              ...
              <div class="student" v-if="nationalityOfStudents">
                ...
                <p class="number">
                  {{ schoolDetail.nationalityOfStudents }}个国家
                </p>
            ...
            <div class="processBox">
              ...
              >
                ...
                <el-form-item prop="inputNumber">
                  <el-input
                    placeholder="手机号码"
                    v-model="formInline.inputNumber"
                  ></el-input>
                </el-form-item>
                <el-form-item>
                  <el-select
                    placeholder="在读年级"
                    v-model="formInline.inputGrade"
                  >
              ...
              <el-form :model="formInline">
                <el-form-item class="remark">
                  <el-input
                    type="textarea"
                    v-model="formInline.inputRemark"
                    placeholder="备注"
                  ></el-input>
                </el-form-item>
              </el-form>
            ...
            <el-row style="width: 87%;margin: 0 auto;">
              ...
              >
                ...
                >
                  ...
                  />
                  <p class="chineseName">
                    {{ item.schoolName | ellipsisName }}
                  </p>
                  <p class="englishName">
                    {{ item.schoolEnglishName | isZero | ellipsisEname }}
                  </p>
                </div>
              </el-col>
            </el-row>
              ...
              >
                <div class="popover_box">
                  ...
                  <div>
                    ...
                    <a
                      class="popover_a"
                      :href="xinxueshuoSite + 'companyDetail?id=' + item.t02"
                      target="_blank"
                      >查看更多</a
                    >
                  </div>
                ...
                <a
                  class="schoolTranslateH1_ula"
                  @mouseover="companyDetail_val(item.t02)"
                  :href="xinxueshuoSite + 'companyDetail?id=' + item.t02"
                  target="_blank"
                  slot="reference"
                  >{{ item.tname }}</a
    ...
    <div class="feedback">
      如您在浏览过程中发现数据有误，可通过<span @click="feedback">意见反馈</span
      >提交给我们，我们将及时处理
    </div>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        inputName: [{ required: true, message: "名字不能为空" }],
        inputNumber: [{ validator: validatePhone }]
      },
      asyncObject: "",
      flag: false,
      pcSee: false,
    ...
    getDetail() {
      ...
      getSchoolDeatail({
        ...
        if (
          ...
          (that.schoolDetail.teacherNum == 0) &
          (that.schoolDetail.foreignTeacherNum == 0) &
          (that.schoolDetail.nationalityOfStudents == null) &
          (that.schoolDetail.teacherStuRatio == "0")
        ) {
          that.teacher = false;
        } else {
          that.teacher = true;
        ...
        // 招生信息
        if (
          that.schoolDetail.studentEnrollmentVo == null ||
          (that.schoolDetail.studentEnrollmentVo.Kindergarten == "" &&
            that.schoolDetail.studentEnrollmentVo.primarySchool == "" &&
            that.schoolDetail.studentEnrollmentVo.JuniorHighSchool == "" &&
            that.schoolDetail.studentEnrollmentVo.heightSchool == "")
        ) {
          ...
          that.admissionList2 =
            that.schoolDetail.studentEnrollmentVo.PrimarySchool;
          that.admissionList3 =
            that.schoolDetail.studentEnrollmentVo.MiddleSchool;
          that.admissionList4 =
            that.schoolDetail.studentEnrollmentVo.HighSchool;
        ...
        // 去除最后空格
        that.approveSplit = that.approveSplit.slice(
          0,
